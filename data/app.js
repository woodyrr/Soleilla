...
function renderCartItems(){
    ...
    cart.forEach((item) => {
        cartItov.innerHTML += `
        <div class="z-[9999] cartItov flex text-white justify-between pt-10 px-[2%] duration-300" >
            <img class="w-[15%] lg:w-[18%] duration-500 hover:bg-white" src="${item.img}" alt="${item.name}" srcset="">
            <div class="flex flex-col justify-center items-center gap-2">
                <h3 class="text-center text-sm px-6 font-semibold lg:text-base">${item.name}</h3>
                <div class="flex justify-center items-center gap-1">
                    <div>QTY:</div>
                    <button class="px-4 lg:px-6 2xl:py-1 bg-orange-300 rounded-lg text-lg lg:text-xl text-black" onclick="changenumberofunits('minus', ${item.id})">-</button>
                    <div class="lg:text-lg">${item.units}</div>
                    <button class="px-4 lg:px-6 2xl:py-1 bg-orange-300 rounded-lg text-lg lg:text-xl text-black" onclick="changenumberofunits('plus', ${item.id})">+</button>
                </div>
            </div>
